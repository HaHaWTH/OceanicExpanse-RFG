...
public enum OceanType implements IStringSerializable {
    ...
    final String name;
    @Nonnull
    DoublePredicate temperatureChecker;
     ...
     */
    public boolean registerBiome(@Nonnull final Biome biome, final int weight) {
        return registerBiome(new BiomeManager.BiomeEntry(biome, weight));
    }
    ...
    public Biome getRandomBiome(@Nonnull final IntUnaryOperator rand) {
        if (fallbackIndex < 0) {
            if (biomes.isEmpty()) return Biomes.OCEAN;
